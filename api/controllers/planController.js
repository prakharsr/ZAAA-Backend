...
function getCurrentPlanCost(firm,plan){
    var now  =  new Date();
    quartersUsed = getQuarter(now) - getQuarter(firm.plan.createdOn);
    plan.cost -= plan.cost * quartersUsed/4;
}
module.exports.getPlans = function(request,response){
    var user = response.locals.user;
    var plans = Plan.find({},null, {sort: {cost: 1}},function(err, plans){ 
        if(err){
            ...
            if(plans.length == 0){
                ...
                if(firm.plan.planID == null || firm.plan.planID == undefined){
                    ...
                    plans.map(plan => {
                        if(plan._id == firm.plan.planID){
                            getCurrentPlanCost(firm,plan);
                        }
                    });
                    response.send({
                        success:true,
